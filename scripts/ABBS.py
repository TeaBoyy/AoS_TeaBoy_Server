...
# from pyspades.contained import input_data, weapon_input, set_tool, set_color, make_color
...
try:
    ...
    BOT_NUM_NAME    = True    
    CPU_LV          = [80,2]  
    BOTMUTE         = False    # bot chat off
    DANSA_CALC_NUM  = 5        
    KEIRO_TANSAKU_NUM = 5      
    ENEMY_SEARCH_FPS  = 4
    BOT_UPDATE_RATE   = 1      #60/n FPS 
    LVCHANGE_RECONNECT= False 
    LV_PRINT = False 			
    ...
    def apply_script(protocol, connection, config):
        class BotProtocol(protocol):
                        ...
                        if VSBOT_INTEL:
                            ...
                            if flagger is not None:
                                if flagger.world_object:
                                    if flagger.hp>0 and  flagger.world_object.position.x > LINE_X and not self.gamewon:
                                        flagger.gamewin()
                        for player in self.blue_team.get_players():
                            plnum +=1
                            if player.world_object:
                                if player.hp>0 and  player.world_object.position.x > LINE_X and not self.gamewon:
                                    if VSBOT_alone:
                                        player.gamewin()
                                    else:
                                        plclr+=1
                ...
                if BOT_ADD_PATTERN==0:
                    if BOT_IN_BOTH:
                        ...
                        if bot_over == 0: 
                            ...
                            if bot_over >1:#bot 2ijou������
                                return -1,-1
                            elif bot_over > 0:#bot ������
                                return -1,0
                            elif bot_over < -1:#bot 2ijou���Ȃ�
                                return 1,1
                            elif bot_over < 0:#bot ���Ȃ�
                                return 1,0
                    else:
                        if green_bot > BOT_ADD_NUM: #bot������
                            return 0,-1
                        elif green_bot < BOT_ADD_NUM: #bot���Ȃ�
                            return 0,1
                        else:#bot����薳��
                            return 0,0
                elif BOT_ADD_PATTERN==1:#�����t���Œ�
                    if BOT_IN_BOTH:
                        blue = blue_human + blue_bot
                        green = green_human + green_bot
                        ...
                        balance = blue-green
                        if balance >=1:#������
                            if botnum_over>0: #bot������
                                return -1,0
                            elif botnum_over<0:#bot���Ȃ�	
                                return 0,1
                            else:
                                if balance>1:
                                    return -1,1
                        ...
                        if green_bot > BOT_ADD_NUM: #bot������
                            ...
                            return 0,0
                elif BOT_ADD_PATTERN==2: #bot + human���v�l�Œ�
                    blue = blue_human + blue_bot
                    green = green_human + green_bot
            ...
            def count_human_bot(self):	#spectate �͏���
                blue_human  = 0
                blue_bot    = 0
                green_human = 0
                green_bot   = 0
        ...
        class BotConnection(connection):
            ...
            def bot_property(self):
                ...
                if weapon_rdm>0.4:
                    ...
                    if weapon_rdm>0.3:
                        ...
                        self.z_add= gauss(0.1,0.2*(1-self.cpulevel)/3)	#���l��SMG�ł�HS�_��	
                else:
                ...
                if ARENAmode or TDMmode or VSBOTmode:
                    self.positive_attacker= True	#�ڕW�_���v���C���[��D�悵�čU��
                else:
                    self.positive_attacker=random.random()>0.1 #False�Ȃ�U���󂯂Ȃ�������G���Ȃ��ŖڕW�_�ɓˌ�
            ...
            def cast_sensor(self,pos,SENSOR_LENGTH,orientation):	#position�߂��Ȃ��̂Œ���
            ...
            def canseeY2(self,mypos,ori, d): #cast_ray���M�p�Ȃ�Ȃ��̂Ŏ���
            ...
            def canseeY(self,mypos,tgt_head, No_need_compx=False): #No_need_compx�Ȃ�w����W�̂݁@False�Ȃ�w����W�𓪂Ƃ��ē����܂Ōv�Zcast_ray������̕����g��
    ...
    #	2		#	B		#	B	 X	#	B		#	B	 X	#	B	r	#
    ################################################## 
                if c0r + c0l +  c1r + c1l +  c2r + c2l ==0  : # �O����Q�Ȃ�
                    pattern = 0
                    AP+=0

                elif c1r + c1l +  c2r + c2l== 0 :# -1 ���Ⴊ�߂Βʂ��
                    pattern = -1
                    AP+=AP_CROUCH

                elif (0<c0r<=c1r+0.1 and 0<c0r<=c2r+0.1) or (0<c0l<=c1l+0.1 and 0<c0l<=c2l+0.1) : # DAME �������őO����Q�L�蔻��
                    pattern = 6661
                    AP+=0
                elif (0<c0r<=c1r+0.1 and c2r==0) or (0<c0l<=c1l+0.1 and c2l==0) : # DAME �������őO����Q�L�蔻��
                    pattern = 6662
                    AP+=0
                elif (0<c0r<=c2r+0.1 and c1r==0) or (0<c0l<=c2l+0.1 and c1l==0) : # DAME �������őO����Q�L�蔻��
                    pattern = 6663
                    AP+=0
                if 0<pattern<666:
    #		A	#		B	#		C	#		D	#	
    ...
    ################################################## 
                ...
                if pattern ==1 or pattern == 2:
                    ...
                    if pattern ==1:
                        if min(c2r,c2l)==0:
                            dist = max(0,max(c2r,c2l)-UEtemae)	#�y��u���b�N���ݒn�_��1.5bk��O����T��
                        else:
            ...
            def route_optimization(self,tpos,data=None,avoid=False):
                ...
                mindist = 999
                if data:
                    for choiced_route in routechoce:#�����胋�[�g�����ɒT��
                        if data[choiced_route] !=[]:
                            ...
                            if mindist>tempmin[0]:
                                ...
                                direct=tempmin[2]
                                if avoid:
                                    direct=tempmin[2]*-1 #�O���l�[�h��������]
                                num=tempmin[1]
                                pos=junkai_route[routenum][num]
                                kouho = [routenum, direct, num, pos]
                                if sect*ARENA_JUNKAI_SECT<dist_dakyou: #�ŒZ����Ȃ��Ă�������x�őË�
                                    break
                else:
                    for choiced_route in routechoce:#�S���[�g�����ɒT��
                            templistP=[]
                            templistT=[]
                            for num, pos in enumerate(junkai_route[choiced_route]):
            ...
            def near_point_mem(self,point):
                ...
                routenum=len(junkai_route)
                mem=[]
                for route in range(routenum):#�S���[�g�����ɒT��
                    mem.append([])
                ...
                return mem
            #type: 1=step, 2=dash,jump, 3=reload, 4=fire
            def noise(self,type): 
            ...
            def grenade_pin(self):
                if seconds() - self.toolchangetime>0.5 and self.tool == GRENADE_TOOL and not self.grenade_keeping:
                    if self.aim_at and self.enemy_lost_temp != None:
                        tgtpos = copy(self.enemy_lost_temp)
                        botpos = self.world_object.position.get()
                        d = self.distance_calc(botpos,tgtpos)
                        if d>15 or self.canseeY(self.world_object.position,Vertex3(tgtpos[0],tgtpos[1],tgtpos[2]+2))<0:
                            ...
                            if phi:
                                ...
                                if random.random()<0.2:time=0.9 #���܂ɂ���������
            ...
            def late_gosa(self): #�_�_�����܂Ŏ��Ԃ�������V�X�e��
                off_r = (self.xoff_okure**2+self.yoff_okure**2)**0.5
                if off_r==0:
                    return 0, 0
                d_okure=100/off_r *((1-self.cpulevel)/2+0.5)				#�����̌W���͌����̗]�n����
                d_backokure = 1.0/d_okure							#1�T�C�N����1/n�������S�ɋ߂Â�
            ...
            def rotate_gosa(self):#����ʂɉ����Č덷�~��+����񎞂͐���ʃI�[�o�[
            ...
            def moving_target_gosa(self):
                ...
                self.vel=max( min((1-self.cpulevel)*5 , self.vel) , (1-self.cpulevel)*(-5))	#���ڕW�̑��x��F�W��
                ...
                if self.aim_at and self.aim_at.world_object and not (self.gre_avoiding and not self.gre_ignore): #�ˌ��ΏۓG�v���C���[�F�����
                    ...
                    if self.grenade_keeping and self.tool == GRENADE_TOOL:	#grenade�s�������ێ���
                        if self.enemy_lost_temp ==None or distance_to_aim<15 or random.random()<0.005:#�ڕW15bk���ڋ�or��m��
                            ...
                            self.grenade_throw_orienation= self.grenade_calc(self.enemy_lost_temp,True) #�ڕW���W�ɑ΂��ێ��p�x�C��
                    if canseepos>=0:  #�ڕW���F�\��
                        self.enemy_lost_temp=aim_at_pos.get()
                        self.aim_quit=100

                        #����s������
                        if self.aim_at.tool==WEAPON_TOOL: #�G���e������
                            diff = target_obj.orientation - self.aim	
                            diff = diff.length_sqr()								#�G�_�_�����Ɣމ�̂Ȃ��p
                            if diff > 0.01 and self.cpulevel>0.3:
                                p_dot = target_obj.orientation.perp_dot(self.aim)
                                if 0.1 > p_dot > -0.1:
                                    if self.cpulevel>0.5:
                                        if random.random()<self.cpulevel and random.random()<30/distance_to_aim**2 and random.random() > 0.95: #�W�����v���
                        ...
                        #�_�_����
                        #�_�_�덷
                        self.moving_target_gosa()#���ڕW��F
                        aim_at_pos_future = Vertex3(aim_at_pos.x + self.movezure_x, aim_at_pos.y + self.movezure_y, aim_at_pos.z+self.movezure_z) #���ڕW�̗\���ʒu��F��ʒu
                        if canseepos==0:	#����������ꍇ
                            aim_at_pos_future.z += self.z_add #���_���̏ꍇ�𔽉f
                        else:
                            aim_at_pos_future.z += canseepos #�_�_���������̌�������Ɉړ�

                        self.aim.set_vector(aim_at_pos_future)
                        self.aim -= pos
                        distance_to_aim = self.aim.normalize()

                        x_back, y_back = self.late_gosa()	#�_�_�̎����x��
                        self.tebure_gosa()					#��Ԃꕪ
                        self.rotate_gosa() 					#���ߐ���p�ˑ��덷
    #					print "%.3f, %.3f, %.3f, (%.2f,%.2f,%.2f) "%(self.xoff_tebure, self.xoff_okure, self.large_omega_x,self.movezure_x,self.movezure_y,self.movezure_z)
                        ...
                        theta += self.xoff_tebure + self.xoff_okure + self.large_omega_x*self.omega	#�e�v�f�덷��ڕW�����ւ̐��m�p�x�ɉ��Z
                        ...
                        # TODO: this is a test for nade launchers to aim higer based on distance
                        test_up_z = 0
                        offset = 0.002
                        if distance_to_aim > 20:
                            test_up_z = distance_to_aim * offset
                        elif distance_to_aim > 5:
                            test_up_z = distance_to_aim * -offset/2
                        else:
                            test_up_z = -0.35
                        ...
                        if diff > 0.001:
                            ...
                            ori.set_vector(self.target_orientation)
                        if self.grenade_keeping: #grenade�s�������ێ����͎ˊp�ɏC��
                            obj.set_orientation(*self.grenade_throw_orienation)

                        #�U������
                        if self.tool != GRENADE_TOOL: 							#grenade�����Ă��Ȃ��ꍇ�A��m���ŃO���l�[�h�g�����Ƃ���B�މ䍂�፷������Ɗm����
                        ...
                        if self.battlecrouching and self.tool == WEAPON_TOOL:
                            ...
                            if crouchcansee>=0: #���Ⴊ��ł�������Ȃ�
                                defcrouch=True #���Ⴊ�݂��f�t�H
                                self.input.add('crouch')
                    else: #�ڕW�ǔ������������Ȃ���
                        if self.aim_quit<=60:
                            if random.random()**2/60>self.cpulevel:
                                self.bot_reload()
                        if self.enemy_lost_temp != None and self.grenades>0:	#�ڕW���X�g���Օ����ɉB�ꂽ�\���������̂ō��m���ŃO���l�[�h�𓊂���
                        ...
                        if self.aim_quit<=0:
                            ...
                            self.aim_at=None #�ڕW�����Ȃ��󋵂��p���ɂ��ڕW�w�����, ���[�h�ɂ���Ă̓��X�g�n�_�֑{���J�n
                    ...
                    if self.reload_wait<=0:
                        self.bot_reload()
                    self.xoff_okure, self.yoff_okure = self.new_gosa(False) #�덷�������Z�b�g
                    px,py,pzo = pos.get()
                    self.tgt_pos_update()
                    if self.stopmotion:
                        ...
                        #�ڕW������������Q�T�m
                        if self.long_recg>=10: #n��Ɉ�񂾂��������T�m
                            self.long_recg=0
                            if pz >= 60:#�C
                                if TGT_ORIENT.y==0:
                                ...
                                dist=0
                                for ii in range(DANSA_CALC_NUM): #����2�̒i����n�i���v�Z
                                ...
                                if AP>0:#��������Q�������A�œK�j�H�T������
                                    nowori = Vertex3(ox,oy,oz) #���ݐi�s����
                                    z_plus = 0
                                    APn=0
                                    dist=0
                                    self.front_rcog = [[-1]*64, [-1]*64] # [R,L]
                                    ...
                                    if APn>=AP-15:
                                        ...
                                        AP=APn
                                    self.world_object.position.set(px,py,pz)	
                                    if self.world_object.velocity.z**2<0.01: #������Ԃł͐i�s�������䂵�Ȃ�
                                        if AP>666:	#�i�s�s�\�j�H
                        ...
                        if AP>=0:					#�Z������Q���m
                            ...
                            frt = 1
                            jumping = False
                            if pattern+umi==0: #�O�����ߏ�Q�Ȃ�
                                self.input.add('sprint')
                            elif pattern+umi==1: #�O������1�i��
                                self.input.discard('sprint')
                            elif pattern+umi==-1: #���Ⴊ�߂Βʂ��
                                self.input.add('crouch')
                            elif pattern+umi==2: #�W�����v�ł������
                                if dist<1.5:
                                    jumping = True
                                    self.input.add('jump')
                                else:
                                    self.input.discard('sprint')
                            else:	#���S�ɐ��ʂ��ǂȏ󋵁@���E5�u���b�N�����ɂ��ꂽ�琳�ʂ��J���邩�𔻒�
                                SENSOR_LENGTH_I=10
                                for i in range (5):
                                    if TGT_ORIENT.y==0:
                            ...
                            if  (self.world_object.velocity.x**2 + self.world_object.velocity.y**2)**0.5 < 0.0001: #��~��
                                self.input.discard('sprint')	#�Ƃ肠��������̂��~�߂�
                                if self.protocol.map.get_solid(px,py,pz): #���Ⴊ��łȂ����̓��̈ʒu�Ƀu���b�N������
                                    if not self.protocol.map.get_solid(px,py,pz+1): #���Ⴊ��łȂ����̓��̈ʒu+1�Ƀu���b�N���Ȃ�
                                        self.input.add('crouch')
            ...
            def spadeing(self):
                ...
                type = SPADE_DESTROY	#���ʒu�Ƀu���b�N����ΉE�N���j��
                if tgtblockl1 is None and tgtblockr1 is None and tgtblockc1 is None:
                    ...
                    tgtblockl0 = self.posset_castray((Lx,Ly,pz),3)	#���ʒu�Ȃ������ʒu����ł��E�N���j��
                    if tgtblockl0 is None and tgtblockr0 is None and tgtblockc0 is None:
                        ...
                        tgtblockl2 = self.posset_castray((Lx,Ly,pz+2),2)	#�����ʒu�Ȃ������ʒu����Ȃ獶�N���j��
                ...
                for player in self.team.other.get_players():
                    if player.world_object:
                        ...
                        if dist_p<130:
                            ...
                            diff_vect = (gaiseki[0]**2 + gaiseki[1]**2 + gaiseki[2]**2)**0.5 #������x�Ȃ��p�̏�������������I��
                            if diff_vect<3.5:
                ...
                for calc_do in range(int(128 / d_calc)):
                    ...
                    if _z >= 0:
                        if self.protocol.map.get_solid(*round_pos): 
                            return round_pos
                    if farest_dist+4 > distance_fly and distance_fly > nearest_dist -4: #�ŋߖT�G�v���C���[�ɏ\���ڋ߂����瓖���蔻��v�Z�J�n
                        if self.team != None:
                                    ...
                                    if self.world_object.crouch:
                                        if voy==0:
                                            ...
                                            if -0.3 < z < 0.3 :# z axis head shot
                                                ...
                                                hidari=  x/k+0.3/vox
                                                migi  =  x/k-0.3/vox
                                                if min(mae,usiro) < y < max(mae,usiro):#�O������������蒆�S+-0.3bk
                                                    if min(migi,hidari) < y < max(migi,hidari) :#HS����@��̏㉺���͖���
                                                        hit = 1
                                            elif  0.3 <= z < 1.3 :# z axis body or asi shot:
                                                mae   = -k*x+0.2/voy
                                                hidari=  x/k+0.4/vox
                                                migi  =  x/k-0.4/vox
                                                if min(migi,hidari) < y < max(migi,hidari) :#���ʕ���������	��͖���
                                                    if 0.3 <= z < 0.9 :# z axis body shot:
                                                        ketu = -k*x-0.7/voy
                                                        if min(mae,ketu) < y < max(mae,ketu):#�O������������蒆�S+-0.2bk
                                                            hit = 2
                                                    elif 0.9<= z < 1.3:# z axis asi shot:
                                                        kakato = -k*x-0.4/voy
                                                        hidariutiasi=  x/k+0.15/vox
                                                        migiutiasi  =  x/k-0.15/vox
                                                        if min(mae,kakato) < y < max(mae,kakato):#�O������������蒆�S+-0.2bk
                                        ...
                                        if voy==0:
                                            ...
                                            if -0.3 < z < 0.3 :# z axis head shot
                                                ...
                                                if min(mae,usiro) < y < max(mae,usiro):#�O������������蒆�S+-0.3bk
                                                    if min(migi,hidari) < y < max(migi,hidari) :#HS����@��̏㉺���͖���
                                                        hit = 1
                                            elif  0.3 <= z < 2.3 :# z axis body or asi shot: #���Ⴊ�݂��l�����ĂȂ�
                                                mae   = -k*x+0.2/voy
                                                usiro = -k*x-0.2/voy
                                                hidari=  x/k+0.4/vox
                                                migi  =  x/k-0.4/vox
                                                hidariutiasi=  x/k+0.15/vox
                                                migiutiasi  =  x/k-0.15/vox
                                                if min(mae,usiro) < y < max(mae,usiro):#�O������������蒆�S+-0.2bk
                                                    if min(migi,hidari) < y < max(migi,hidari) :#���ʕ���������	��͖���
            ...
            def fire_weapon(self):
                if self.world_object_alive_onpos():
                    ...
                    if self.weapon == SMG_WEAPON:
                        ...
                        cooltime = 1.1
                    if now-self.last_fire>cooltime and self.tamakazu>0 and now > self.reloadfin:#�ˌ��\�ȃ^�C�~���O������ �c�e�m�F
                        if self.smg_shooting>0 or now-self.last_fire>self.next_fire_time:#�\��Ԋu�󂯂�������iSMG�Ȃ�A�ˉj
                            self.next_fire_time=uniform(0,(1.2-self.cpulevel)*2.5)+1.1	#���̎ˌ��̗\��Ԋu
                            if self.weapon == SMG_WEAPON:
                            ...
                            if self.protocol.bot_damage == True:
                                blkhit = self.bot_bullet() #�ˌ����s�A�����蔻��v�Z�i�v���C���[�D��A�u���b�N�͎����Łj
                                if blkhit:#�u���b�N�Փ˔���L��Δj�󏈗�
                                    if blkhit in self.damaged_block:
                                        self.damaged_block.remove(blkhit)
                                        self.fire_block_break(*blkhit)
                                    else:
                                        self.damaged_block.append(blkhit) #�߂�ǂ������̂ŕ����킸2���ŕ���
                                if self.weapon == SHOTGUN_WEAPON: #SG�̏ꍇ�����7�񓯈ꏈ��
                                    for n in range(7):
                            ...
                            recoil_control=0
                            if random.random()<self.cpulevel:#���������R�C���R���g���[��
                            ...
                            self.target_orientation.set_vector(self.aim) #���R�C�����orientation�ɕύX iminasage
                            xoff,yoff = self.new_gosa(True) #�덷�������Z�b�g
                            self.xoff_okure += xoff
                            self.yoff_okure += yoff #self.new_gosa(True) #�덷�������Z�b�g
            ...
            def on_spawn(self, pos):
                ...
                sita= min(99,max(0,teamcpulv[0]-teamcpulv[1]))
                ue  = min(100,max(1,teamcpulv[0]+teamcpulv[1]))
                if self.protocol.reflesh_bot and not sita<self.cpulevel*100<ue:	#lv������͈͓̔��ɖ���
                    ...
                    if LVCHANGE_RECONNECT:
                        ...
                        callLater(0.01,self.protocol.add_bot,self.team)#�I�ɓ���Ȃ����ă��x����ς��Ă���
                    else:
                        self.cpulevel=uniform(sita/100.0,ue/100.0)#�I�ɏo���肵�Ȃ��œ������l�����ς���
                        self.bot_property()
                    lvsum=0
                    numsum=0
                    for bot in self.protocol.players.values():
                        if bot.local and bot.team==self.team:
                            ...
                            numsum+=1.0
                    if not sita<lvsum/numsum<ue:#�`�[���S�̂̕��ϒl���K��l�ɖ����ꍇ�͕ω����p��
            ...
            def on_team_join(self, team):
                if not self.local:
                    ...
                    if BOT_IN_BOTH:	#�`�[���Ԃ̐l�Ԕ䗦���΂��Ă�ꍇ�A�l�ԑ����`�[�����`�[��FULL�����ɂ���
                        blue_human, blue_bot, green_human, green_bot = self.protocol.count_human_bot()
            ...
            def FCcontact(self,ori):		#���ߒe���m�Ō��J�n	
                for player in self.team.other.get_players():
                    if player.local:#�Gbot�S���̍��W���m�F���āA�ː��ƂȂ��p�����ȉ��Ȃ炻��bot�̃w�C�g���󂯂�
                        epos= player.world_object.position
                        pos=self.world_object.position
